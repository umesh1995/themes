...
if (!function_exists('simple_php_captcha')) {
    function simple_php_captcha($config = array())
    {
        ...
        if (strpos($SERVER['SCRIPT_FILENAME'], $SERVER['DOCUMENT_ROOT'])) {
            $image_src = substr(__FILE__, strlen(realpath($SERVER['DOCUMENT_ROOT']))).'?_CAPTCHA&amp;t='.urlencode(microtime());
            $image_src = '/'.ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
        } else {
            $SERVER['WEB_ROOT'] = str_replace($SERVER['SCRIPT_NAME'], '', $SERVER['SCRIPT_FILENAME']);
            $image_src          = substr(__FILE__, strlen(realpath($SERVER['WEB_ROOT']))).'?_CAPTCHA&amp;t='.urlencode(microtime());
            $image_src          = '/'.ltrim(preg_replace('/\\\\/', '/', $image_src), '/');
        }
